  ...
  namespace '/api/v1' do #помню что v1, v2, v3 и т.д. так положено писать про версию апи
    ...
    namespace '/api/public/v1' do
      get '/timetables' do
        ...
        timetables.map { |timetable| TimetableAppoint.new(timetable) }.to_json
      end
  
      get '/tickets/user/:id' do
        tickets = Ticket.where(user_id: params[:id])
        tickets.map { |ticket| TicketAppoint.new(ticket) }.to_json
      end
    ...
    namespace '/api/admin/v1' do
      ...
      get '/routes' do
        routes = Route.all
        routes.map { |route| RouteAppoint.new(route) }.to_json
      end
